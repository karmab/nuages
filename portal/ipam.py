...
class Ipam:
	...
	r = requests.get(envurl,headers=headers,auth=(username,password))
	results = r.json()
	...
	r = requests.get(rolesurl,headers=headers,auth=(username,password))
	results = r.json()
	results = results['results']
	for r in results:
		name=r['code']
	...
	r = requests.get(operatingsystemsurl,headers=headers,auth=(username,password))
	results = r.json()
	results = results['results']
	for r in results:
		name=r['code']
	...
	r = requests.get(dnszonesurl,headers=headers,auth=(username,password))
	results = r.json()
	results = results['results']
	for r in results:
		name=r['name']
	...
	r = requests.get(mtypesurl,headers=headers,auth=(username,password))
	results = r.json()
	results = results['results']
	for r in results:
		name=r['name']
	...
	r = requests.get(vlansurl,headers=headers,auth=(username,password))
	results = r.json()
	results = results['results']
	for r in results:
		name=r['name']
	...
	r = requests.get(projectsurl,headers=headers,auth=(username,password))
	results = r.json()
	results = results['results']
	for r in results:
		name=r['name']
	...
	r = requests.post(createurl,postdata,auth=(username,password))
	results = r.json()
	...
	r = requests.post(ipurl,postdata,auth=(username,password))
	results = r.json()
	...
	while not found:
		r = requests.get(nexturl,headers=headers,auth=(username,password))
		results = r.json()
	...
	#get ifaceid
	ipurl = "http://%s:%s/ifaces/" % (host,port)
	r = requests.get(ipurl,headers=headers,auth=(username,password))
	results = r.json()
	results = results['results']
	for r in results:
		if r['machine'].endswith("/%s/" % machineid):
			...
			mask=r['mask']
	#get vlan name
	r = requests.get(vlan,headers=headers,auth=(username,password))
	results = r.json()
	...
	r = requests.put(ipurl,postdata,auth=(username,password))
	#results = r.json()
	#return results
