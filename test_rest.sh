...
# LIST ONE VM
#curl -k -u $username:$password -H "Content-Type: application/json" -X GET http://192.168.8.6/nuages/api/v1/vm/Overlord
#exit 0

#START A VM 
#curl --dump-header - -k -u $username:$password -H "Content-Type: application/json" -X POST  http://192.168.8.6/nuages/api/v1/vm/satellite6/start
#exit 0 
...
#CREATE A NEW VM
curl  --dump-header - -k -u $username:$password -H "Content-Type: application/json" -d  '{"name":"alibop", "profile":"/nuages/api/v1/profile/basic6"}' -X POST http://192.168.8.6/nuages/api/v1/vm
